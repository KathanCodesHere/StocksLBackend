...
export const approvePendingUser = async (req, res) => {
  try {
    ...
    const uniqueId = `octa${(Math.floor(Math.random() * 1000) + 1000)
      .toString()
      .substr(1)}`;
...
export const rejectPendingUser = async (req, res) => {
  try {
    ...
    const emailTemplate = userRejectionTemplate(
      existingUser.name,
      rejection_reason
    );
  ...
  try {
    ...
    // 2. AGAR ADMIN NAHI MILA TO USERS TABLE MEIN
      ...
      // Fir unique_id se check
        ...
        const [usersByUniqueId] = await pool.execute(userByUniqueIdQuery, [
          email,
        ]);
    ...
    if (userType === "user" && user.status !== "approved") {
      return sendError(
        res,
        401,
        `Account status: ${user.status}. Please wait for approval.`
      );
    }
...
export const deleteUserByAdmin = async (req, res) => {
  try {
    ...
    await pool.execute(`DELETE FROM kyc WHERE user_id = ?`, [userId]);
